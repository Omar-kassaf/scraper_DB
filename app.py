...
def save_user_progress_to_firestore(username, progress):
    doc_ref = db.collection('user_progress').document(username)
    doc_ref.set(progress)

def load_user_progress_from_firestore(username):
    doc_ref = db.collection('user_progress').document(username)
    doc = doc_ref.get()
    if doc.exists:
        return doc.to_dict()
...
def save_user_progress(username, keywords, selected_option, task_complete):
    ...
    save_user_progress_to_firestore(username, progress)
